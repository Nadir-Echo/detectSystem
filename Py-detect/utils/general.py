...
def file_age(path=__file__):
    ...
    return dt.days  # + dt.seconds / 82000  # fractional days
...
def check_img_size(imgsz, s=32, floor=0):
    ...
    if isinstance(imgsz, int):  # integer i.e. img_size=224
        new_size = max(make_divisible(imgsz, int(s)), floor)
    else:  # list i.e. img_size=[224, 480]
...
def check_amp(model):
    ...
    im = f if f.exists() else 'https://ultralytics.com/images/bus.jpg' if check_online() else np.ones((224, 224, 3))
    try:
...
def xywhn2xyxy(x, w=224, h=224, padw=0, padh=0):
...
def xyxy2xywhn(x, w=224, h=224, clip=False, eps=0.0):
...
def xyn2xy(x, w=224, h=224, padw=0, padh=0):
...
def segment2box(segment, width=224, height=224):
