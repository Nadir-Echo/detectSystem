...
f = (224, 224)  # 输入图片的大小 默认640(pixels)
...
def detect_(img):
    ...
    # gn = torch.tensor([224,224,224,224])
    # print(gn)
    for i, det in enumerate(pred):  # per image 每张图片
        if len(det):
            ...
            for *xyxy, conf, cls in reversed(det):
                ...
                # xyxy = (xywhn2xyxy(torch.tensor(xyxy).view(1, 4), w=224, h=224, padw=0, eps=0)).view(-1).tolist()
                # xywh = (xyxy2xywhn(torch.tensor(xyxy).view(1, 4), w=224, h=224, clip=False, eps=0.0)).view(-1).tolist()
